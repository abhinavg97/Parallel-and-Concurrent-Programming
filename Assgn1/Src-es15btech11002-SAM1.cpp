...
void work(int id)
{
	...
	for(number_to_check = id ; number_to_check<=N;number_to_check+=m)        // checking as suggested in the assignment question
	{
...
void process()
{
	...
	fd = fopen (string("Primes-SAM1.txt").c_str(), "w+");
	fd2 = fopen(string("Times.txt").c_str(), "a");
	
	fprintf(fd2, "Time taken for SAM1 for n = %d, m = %d is %lldms\n", n, m, (long long)duration.count());
